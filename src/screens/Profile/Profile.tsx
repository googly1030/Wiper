...
export const Profile = () => {
  ...
  const [profile, setProfile] = useState<UserProfile>(() => {
    try {
      ...
      return {
        ...
        phone_number:
          localStorage.getItem("userPhone") || userData.phoneNumber || "",
        address: `${localStorage.getItem("userBlock") || ""} ${
          localStorage.getItem("userApartment") || ""
        }`.trim(),
        occupation: null,
        bio: null,
        birth_date: null,
    ...
    const fetchUserCars = async () => {
      if (activeTab === "vehicles") {
        ...
        try {
          ...
          // Map the car data and add image URLs
          const carsWithImages = carData.map((car) => ({
            ...car,
            image: `https://images.unsplash.com/photo-${Math.floor(
              Math.random() * 1000
            )}?auto=format&fit=crop`,
            // In a real app, you'd use actual car images from your storage
          }));

          setUserCars(carsWithImages);
        } catch (error) {
  ...
  useEffect(() => {
    ...
    if (
      sectionParam &&
      ["profile", "security", "vehicles", "history"].includes(sectionParam)
    ) {
    ...
    const handleStorageChange = () => {
      try {
        ...
        setProfile((prev) => ({
          ...
          email:
            localStorage.getItem("userEmail") || userData.email || prev.email,
          full_name:
          ...
          phone_number:
            ...
            prev.phone_number,
          address:
            `${localStorage.getItem("userBlock") || ""} ${
              localStorage.getItem("userApartment") || ""
            }`.trim() || prev.address,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
      ...
      <div className="relative -mt-10 sm:-mt-16 z-10">
        <svg
          className="fill-white w-full h-20 sm:h-32"
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="flex-1 p-4 sm:p-6 md:p-8 lg:p-10 overflow-y-auto">
              <AnimatePresence mode="wait">
                ...
                {activeTab === "profile" && (
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                      ...
                      <Badge
                        variant="outline"
                        className="bg-gray-50 gap-1 text-gray-500 text-xs sm:text-sm self-start sm:self-auto"
                      >
                ...
                {/* Replace the existing vehicles section with this */}
                {activeTab === "vehicles" && (
                  <motion.div
                    key="vehicles"
                    initial={{ opacity: 0, x: 20 }}
                    ...
                    {loadingCars ? (
                      ...
                      <Card className="p-6 sm:p-8 text-center">
                        <div className="flex flex-col items-center">
                          <CarFront className="w-12 h-12 text-gray-400 mb-4" />
                          <h3 className="text-lg font-semibold mb-2">
                            No vehicles added yet
                          </h3>
                          <p className="text-gray-500 mb-4">
                            Add your first vehicle to get started with our
                            services
                          </p>
                          <Button
                            onClick={() => navigate("/add-car")}
                      ...
                      <CardContent className="p-6 sm:p-8 flex items-center justify-center h-full">
                        <div className="flex flex-col items-center">
                          ...
                          <p className="text-xs sm:text-sm text-gray-500 text-center max-w-xs sm:max-w-sm">
                            Your service history will appear here once you book
                            your first car wash service
                          </p>
                          <Button
